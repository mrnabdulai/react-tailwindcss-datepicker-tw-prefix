...
const ItemTemplate = React.memo((props: ItemTemplateProps) => {
    ...
    const getClassName: () => string = useCallback(() => {
        ...
        return `tw-whitespace-nowrap tw-w-1/2 md:tw-w-1/3 lg:tw-w-auto tw-transition-all tw-duration-300 hover:tw-bg-gray-100 dark:hover:tw-bg-white/10 tw-p-2 tw-rounded tw-cursor-pointer ${textColor} ${textColorHover}`;
    }, [primaryColor]);
...
const Shortcuts: React.FC = () => {
    ...
    return shortcutOptions?.length ? (
        <div className="md:tw-border-b tw-mb-3 lg:tw-mb-0 lg:tw-border-r lg:tw-border-b-0 tw-border-gray-300 dark:tw-border-gray-700 tw-pr-1">
            <ul className="tw-w-full tw-tracking-wide tw-flex tw-flex-wrap lg:tw-flex-col tw-pb-1 lg:tw-pb-0">
