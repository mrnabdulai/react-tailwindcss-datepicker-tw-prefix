...
const Datepicker: React.FC<DatepickerType> = ({
    ...
    const hideDatepicker = useCallback(() => {
        ...
        if (arrow && div && div.classList.contains("tw-block")) {
            div.classList.remove("tw-block");
            div.classList.remove("tw-translate-y-0");
            div.classList.remove("tw-opacity-1");
            div.classList.add("tw-translate-y-4");
            div.classList.add("tw-opacity-0");
            setTimeout(() => {
                div.classList.remove("tw-bottom-full");
                div.classList.add("tw-hidden");
                div.classList.add("tw-mb-2.5");
                div.classList.add("tw-mt-2.5");
                arrow.classList.remove("-tw-bottom-2");
                arrow.classList.remove("tw-border-r");
                arrow.classList.remove("tw-border-b");
                arrow.classList.add("tw-border-l");
                arrow.classList.add("tw-border-t");
            }, 300);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (container && calendarContainer && arrow) {
            ...
            if (containerCenter > screenCenter) {
                arrow.classList.add("tw-right-0");
                arrow.classList.add("tw-mr-3.5");
                calendarContainer.classList.add("tw-right-0");
            }
        }
    }, []);
    ...
    const containerClassNameOverload = useMemo(() => {
        const defaultContainerClassName = "tw-relative tw-w-full tw-text-gray-700";
    ...
    return (
        <DatepickerContext.Provider value={contextValues}>
            <div className={containerClassNameOverload} ref={containerRef}>
                ...
                <div
                    className="tw-transition-all tw-ease-out tw-duration-300 tw-absolute tw-z-10 tw-mt-[1px] tw-text-sm lg:tw-text-xs 2xl:tw-text-sm tw-translate-y-4 tw-opacity-0 tw-hidden"
                    ref={calendarContainerRef}
                >
                    <Arrow ref={arrowRef} />
                    <div className="tw-mt-2.5 tw-shadow-sm tw-border tw-border-gray-300 tw-px-1 tw-py-0.5 tw-bg-white dark:tw-bg-slate-800 dark:tw-text-white dark:tw-border-slate-600 tw-rounded-lg">
                        <div className="tw-flex tw-flex-col lg:tw-flex-row tw-py-2">
                            {showShortcuts && <Shortcuts />}
                            <div
                                className={`tw-flex tw-items-stretch tw-flex-col md:tw-flex-row tw-space-y-4 md:tw-space-y-0 md:tw-space-x-1.5 ${
                                    showShortcuts ? "md:tw-pl-2" : "md:tw-pl-1"
                                } tw-pr-2 lg:tw-pr-1`}
                            >
                                <Calendar
                                    date={firstDate}
