...
const Days: React.FC<Props> = ({
    ...
    const activeDateData = useCallback(
        (day: number) => {
            ...
            if (dayjs(fullDay).isSame(period.start) && dayjs(fullDay).isSame(period.end)) {
                className = ` ${BG_COLOR["500"][primaryColor]} tw-text-white tw-font-medium tw-rounded-full`;
            } else if (dayjs(fullDay).isSame(period.start)) {
                className = ` ${BG_COLOR["500"][primaryColor]} tw-text-white tw-font-medium ${
    ...
    const hoverClassByDay = useCallback(
        (day: number) => {
            ...
            if (dayHover === fullDay) {
                ...
                className = ` tw-transition-all tw-duration-500 tw-text-white tw-font-medium ${bgColor} ${
                    period.start ? "tw-rounded-r-full" : "tw-rounded-l-full"
                }`;
            }
    ...
    const buttonClass = useCallback(
        (day: number, type: "current" | "next" | "previous") => {
            const baseClass =
                "tw-flex tw-items-center tw-justify-center tw-w-12 tw-h-12 tw-lg:w-10 tw-lg:h-10";
            if (type === "current") {
                return cn(
                    baseClass,
                    ...
                    isDateDisabled(day, type) && "tw-line-through"
                );
            }
            return cn(
                baseClass,
    ...
    return (
        <div className="tw-grid tw-grid-cols-7 tw-gap-y-0.5 tw-my-1">
